...
steadyP = np.zeros([n_real, grid_Nx*grid_Ny*grid_Nz])
for i in range(n_real):
	pdata = np.loadtxt('input/SteadyState_P_'+str(i+1)+'.dat')
	steadyP[i,:] = pdata
steadyP = (steadyP-120)/(350-120)
Permtraining = np.tile(steadyP,(int(m/n_real),1))
PermdataX1 = np.reshape(Permtraining,(m, grid_Nz, grid_Nx, grid_Ny))
PermdataX2 = np.transpose(PermdataX1,(0,2,3,1))
PermdataX = np.expand_dims(PermdataX2,axis=4)
...
# Build CNN
perminput = keras.layers.Input(shape=[grid_Nx,grid_Ny,grid_Nz,1])
conv1 = tf.keras.layers.Conv3D(4, [3,3,3], strides=(1, 1, 1), padding='same')(perminput)
bn1 = tf.keras.layers.BatchNormalization()(conv1)
act1 = tf.nn.relu(bn1)
pool1 = tf.keras.layers.MaxPool3D((2,2,2))(act1)
conv2 = tf.keras.layers.Conv3D(8, [3,3,3], strides=(1, 1, 1), padding='same')(pool1)
bn2 = tf.keras.layers.BatchNormalization()(conv2)
act2 = tf.nn.relu(bn2)
pool2 = tf.keras.layers.MaxPool3D((2,2,2))(act2)
conv3 = tf.keras.layers.Conv3D(16, [3,3,3], strides=(1, 1, 1), padding='same')(pool2)
bn3 = tf.keras.layers.BatchNormalization()(conv3)
act3 = tf.nn.relu(bn3)
pool3 = tf.keras.layers.MaxPool3D((2,2,2))(act3)
